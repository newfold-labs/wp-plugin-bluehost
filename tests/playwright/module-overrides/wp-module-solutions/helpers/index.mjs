...
async function setSolution(solution, expiration = 3600, retries = DEFAULT_SET_RETRIES) {
  ...
  if (!fixtureData) {
    fancyLog(`Unknown solution: ${solution}`, 100, 'yellow');
    return {
      ok: false,
  ...
  for (let attempt = 1; attempt <= retries; attempt += 1) {
    try {
      ...
      if (!setResult.ok) {
        ...
        if (verifyResult.ok) {
          fancyLog(`Solution set to ${solution}`, 100, 'green');
    ...
    fancyLog(`Solution setup retry (${attempt}/${retries}) for ${solution}: ${lastReason}`, 100, 'yellow');
    if (attempt < retries) {
      await new Promise((resolve) => setTimeout(resolve, DEFAULT_RETRY_DELAY_MS));
    }
  }

  fancyLog(`Failed to set solution ${solution}: ${lastReason}`, 100, 'yellow');
  return {
    ok: false,
...
async function clearSolutionTransient() {
  try {
    ...
    fancyLog(`Failed to clear solution transient: ${error.message}`, 100, 'yellow');
  }
}
...
async function uninstallPlugin(pluginSlug) {
  try {
    ...
    fancyLog(`Failed to uninstall ${pluginSlug}: ${error.message}`, 100, 'yellow');
  }
}
