...
test.describe('ECommerce Module', () => {
	...
	test.beforeEach(async ({ page }) => {
		...
		utils.fancyLog('OVERRIDE FILE IN PLACE FOR NOT-ECOMMERCE SPEC TEST FILE');
	...
	test.describe('No Store - Store Info', () => {
		test('Store info section does not display', async ({ page }) => {
			...
			await expect(page.locator('.nfd-button[data-store-info-trigger="true"]'), { timeout: 20000 }).not.toBeVisible();
		});
	});
	...
	test.describe('No Woo - Quick Add Product', () => {
		test('Add product not visible', async ({ page }) => {
			...
			await expect(page.locator('.nfd-button[data-quick-add-product-trigger="true"]'), { timeout: 10000 }).not.toBeVisible();
		});
	});
